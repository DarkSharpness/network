...
inline constexpr auto endian_transform(_Int value) -> _Int {
    return std::byteswap(value);
}

template <std::integral _Int>
inline constexpr auto host_to_network(_Int value) -> _Int {
...
inline constexpr auto network_to_host(_Int value) -> _Int {
    if constexpr (__detail::is_big_endian) {
        return value;
    } else {
        return endian_transform(value);
